...
interface ModuleDropdownProps {
  ...
  required?: boolean;
  disabled?: boolean;
  showAddNew?: boolean; // New prop to control add functionality
}
...
export const ModuleDropdown: React.FC<ModuleDropdownProps> = ({
  ...
  required = false,
  disabled = false,
  showAddNew = true // Default to true for backward compatibility
}) => {
    ...
    <div className="relative">
      ...
      {isOpen && (
        <div className="absolute z-50 w-full mt-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-md shadow-lg max-h-60 overflow-hidden">
          ...
          {/* Module List */}
          <div className={`overflow-y-auto ${showAddNew ? 'max-h-40' : 'max-h-52'}`}>
            {loading ? (
          ...
          {/* Add New Module Section - Only show if showAddNew is true */}
          {showAddNew && (
